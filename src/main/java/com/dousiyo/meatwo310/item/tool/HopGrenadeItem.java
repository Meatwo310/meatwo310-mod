...
public class HopGrenadeItem extends Item {
    private final int cooldownTicks;

    public HopGrenadeItem(Properties pProperties, int cooldownTicks) {
        super(pProperties);
    ...
    public InteractionResultHolder<ItemStack> use(Level pLevel, Player pPlayer, InteractionHand pUsedHand) {
        ...
        if (!pLevel.isClientSide) {
            var grenade = new HopGrenade(pPlayer, pLevel);
    ...
    public void appendHoverText(ItemStack stack, Level level, List<Component> tooltip, TooltipFlag flag) {
        tooltip.add(Component.translatable("tooltip.meatwo310.hop_grenade").withStyle(ChatFormatting.GRAY));
